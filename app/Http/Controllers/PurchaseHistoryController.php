    ...
    public function purchase_history_details($id)
    {
        ...
        */
        ...
        foreach($orders as $ork => $orv){
            ...
            $orv->delivery_viewed = 1;
            $orv->payment_status_viewed = 1;
            if($orv->payment_status == "unpaid"){
                Config::$isProduction = true;
                Config::$serverKey = env("midtrans_public_key",null);
                
                $er1 = new Transaction();
                $status = $er1::status($orv->code);
                //if((strpos($status, 'rror') !== false)||(!is_array($status))) {}
                if((is_array($status)) != 1){}
                else{
                    if(($status->transaction_status !== "capture")||($status->transaction_status !== "settlement")){
                        $orv->payment_status = "Paid";
                    }
        ...
        return view('frontend.user.order_details_customer', compact('orders','subtotal','tax','ongkir','order','order_detail'));
    }
